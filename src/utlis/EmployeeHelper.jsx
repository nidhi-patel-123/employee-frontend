...
// Delete employee
export const deleteEmployee = async (employeeId, removeFromTable) => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      if (removeFromTable) removeFromTable(employeeId);
    } else {
      alert("Failed to delete employee.");
    }
  } catch (error) {
    alert(error.response?.data?.error || "Server error while deleting employee");
  }
};

// Employee action buttons
export const EmployeeButtons = ({ employeeId, removeFromTable }) => {
  const navigate = useNavigate();

  return (
    <div className="flex space-x-3">
      <button className="px-4 py-2 bg-teal-600 text-white" onClick={() => navigate(`/admin-dashboard/employees/${employeeId}`)}>View</button>
      <button className="px-4 py-2 bg-blue-600 text-white" onClick={() => navigate(`/admin-dashboard/employees/edit/${employeeId}`)}>Edit</button>
      <button className="px-4 py-2 bg-yellow-600 text-white" onClick={() => navigate(`/admin-dashboard/employees/salary/${employeeId}`)}>Salary</button>
      <button className="px-4 py-2 bg-red-600 text-white" onClick={() => navigate(`/admin-dashboard/employees/leaves/${employeeId}`)}>Leave</button>
      <button className="px-4 py-2 bg-gray-800 text-white" onClick={() => deleteEmployee(employeeId, removeFromTable)}>Delete</button>
    </div>
  );
};
